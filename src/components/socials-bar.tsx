      ...
      <div className='flex gap-2'>
        <div className="flex rounded-full border-2 border-accent p-1 gap-2 lg:absolute lg:left-1/2 lg:-translate-x-1/2">
          {socials.map((social) => (
            ...
            >
              <Image src={`/${social.name.toLowerCase()}-dark.svg`} width={35} height={35} alt={`${social.name} Icon`} className="w-8 h-8 text-background" />
        ...
        <div className="flex rounded-full border-2 border-accent p-1">
          <Link
            ...
            target="_blank"
          >
            <MailIcon className="w-6 text-background" />
            <span className="sr-only">Email</span>
          </Link>
